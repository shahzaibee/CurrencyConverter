...
function startCurrencyConverter() {
    inquirer
        ...
        .then((answers) => {
        const fromCurrency = currencies.find((currency) => currency.name === answers.from);
        const toCurrency = currencies.find((currency) => currency.name === answers.to);
        const amount = answers.amount;
        const convertedAmount = (amount / fromCurrency.rate) * toCurrency.rate;
        console.log(chalk.green(`${amount} ${fromCurrency.name} is equal to ${convertedAmount.toFixed(2)} ${toCurrency.name}`));
